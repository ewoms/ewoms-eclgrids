...
void condWriteDoubleField(std::vector<double>& fieldvector,
    ...
    if (deck.hasKeyword(fieldname)) {
        ...
        if (int(eclVector.size()) != num_global_cells) {
            EWOMS_THROW(std::runtime_error, fieldname << " field must have the same size as the "
...
try
{
    ...
    if (deck.hasKeyword("SPECGRID")) {
        ...
        EWOMS_THROW(std::runtime_error, "Found neither SPECGRID nor DIMENS in file. At least one is needed.");
    }
