...
void printInitMessage(std::ofstream& out, const char* origfilename, std::string direction) {
    ...
    out << "-- This grdecl file is generated from eWoms application 'mirror_grid.cpp'." << std::endl
...
// forward declaration
std::vector<double> getMapaxesValues(const Ewoms::Deck& deck);

/// Mirror keyword MAPAXES in deck
void mirror_mapaxes( const Ewoms::Deck& deck, std::string direction, std::ofstream& out) {
...
void mirror_specgrid( const Ewoms::Deck& deck, std::string direction, std::ofstream& out) {
...
void mirror_coord(const Ewoms::Deck& deck, std::string direction, std::ofstream& out) {
...
void mirror_zcorn(const Ewoms::Deck& deck, std::string direction, std::ofstream& out) {
    const auto& specgridRecord = deck.getKeyword("SPECGRID").getRecord(0);
    std::vector<int> dimensions(3);
    dimensions[0] = specgridRecord.getItem("NX").get< int >(0);
    dimensions[1] = specgridRecord.getItem("NY").get< int >(0);
...
std::vector<int> getKeywordValues(std::string keyword, const Ewoms::Deck& deck, int /*dummy*/) {
    return deck.getKeyword(keyword).getIntData();
}

std::vector<double> getKeywordValues(std::string keyword, const Ewoms::Deck& deck, double /*dummy*/) {
...
template <class T>
void mirror_celldata(std::string keyword, const Ewoms::Deck& deck, std::string direction, std::ofstream& out) {
...
int main(int argc, char** argv)
{
    ...
    const Ewoms::Deck deck(parser.parseFile(eclipsefilename , parseContext));
