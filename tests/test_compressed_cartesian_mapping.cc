...
BOOST_AUTO_TEST_CASE(mapping)
{
    ...
    const std::unordered_map<int, int> cartesian_to_compressed =
        Ewoms::cartesianToCompressed(num_cells, global_cell.data());
    ...
    const std::vector<int> compressed_to_cartesian = Ewoms::compressedToCartesian(num_cells,  global_cell.data());
...
BOOST_AUTO_TEST_CASE(nullmapping)
{
    ...
    const std::unordered_map<int, int> cartesian_to_compressed =
        Ewoms::cartesianToCompressed(num_cells, nullptr);

    for (int i = 0; i < num_cells; ++i) {
        BOOST_CHECK_EQUAL(cartesian_to_compressed.at(i), i);
    }

    const std::vector<int> compressed_to_cartesian = Ewoms::compressedToCartesian(num_cells,  nullptr);
