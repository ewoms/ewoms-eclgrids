...
namespace Ewoms
{
    ...
    private:
        ...
        int interface_(const Grid& grid,
                       const int cellIdx,
                       const Ewoms::FaceDir::DirEnum& faceDir);
    ...
    inline int PinchProcessor<Grid>::interface_(const Grid& grid,
                                                ...
                                                const int cellIdx2)
    {
        const auto cell_faces = Ewoms::UgGridHelpers::cell2Faces(grid);
        int commonFace = -1;
        ...
        if (commonFace == -1) {
            ...
            EWOMS_THROW(std::logic_error, "Couldn't find the common face for cell "
                                                ...
                                                const Ewoms::FaceDir::DirEnum& faceDir)
    {
        ...
        for (auto cellFaceIter = cellFacesRange.begin(); cellFaceIter != cellFacesRange.end(); ++cellFaceIter) {
            int tag = Ewoms::UgGridHelpers::faceTag(grid, cellFaceIter);
            if ( (faceDir == Ewoms::FaceDir::ZMinus && tag == 4) || (faceDir == Ewoms::FaceDir::ZPlus && tag == 5) ) {
                faceIdx = *cellFaceIter;
            }
        }

        if (faceIdx == -1) {
            EWOMS_THROW(std::logic_error, "Couldn't find the face for cell ." << cellIdx);
        }
    ...
    {
        std::vector<int> hf_ix(2*Ewoms::UgGridHelpers::numFaces(grid), -1);
        const auto& f2c = Ewoms::UgGridHelpers::faceCells(grid);
        const auto& cf = Ewoms::UgGridHelpers::cell2Faces(grid);

        for (int c = 0, i = 0; c < Ewoms::UgGridHelpers::numCells(grid); ++c) {
                                                       ...
                                                       const int globalIdx)
    {
        ...
        const int* global_cell = Ewoms::UgGridHelpers::globalCell(grid);
        int idx = -1;
        for (int i = 0; i < nc; ++i) {
    ...
    {
        const int nc = Ewoms::UgGridHelpers::numCells(grid);
        const int nf = Ewoms::UgGridHelpers::numFaces(grid);
        std::vector<double> trans(nf, 0);
        int cellFaceIdx = 0;
        auto cell_faces = Ewoms::UgGridHelpers::cell2Faces(grid);
    ...
    inline std::vector<std::vector<int>> PinchProcessor<Grid>::getPinchoutsColumn_(const Grid& grid,
                                                                                   ...
                                                                                   const std::vector<double>& pv)
    {
        const int* dims = Ewoms::UgGridHelpers::cartDims(grid);
                                                   ...
                                                   NNC& nnc)
    {
        ...
        for (auto& seg : minpvSeg) {
            ...
            if ((ijk1[2]-1) >= 0 && (ijk2[2]+1) < dims[2]) {
                ...
                pinFaces.push_back(interface_(grid, topCell, Ewoms::FaceDir::ZPlus));
                ...
                pinFaces.push_back(interface_(grid, botCell, Ewoms::FaceDir::ZMinus));
