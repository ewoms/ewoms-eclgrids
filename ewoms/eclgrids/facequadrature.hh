    ...
    public:
            ...
            : grid_(grid), face_(face), degree_(degree)
        {
            if (grid_.dimensions > 3) {
                EWOMS_THROW(std::runtime_error, "FaceQuadrature only implemented for up to 3 dimensions.");
            }
            if (degree_ > 2) {
                EWOMS_THROW(std::runtime_error, "FaceQuadrature exact for polynomial degrees > 2 not implemented.");
            }
        }
