...
#include <ewoms/eclgrids/common/volumes.hh>
...
#if HAVE_ECL_INPUT
Ewoms::EclipseGrid createEclipseGrid(const UnstructuredGrid& grid, const Ewoms::EclipseGrid& inputGrid ) {
    ...
    if ((inputGrid.getNX( ) == static_cast<size_t>(dims[0])) &&
        ...
        return Ewoms::EclipseGrid( inputGrid, grid.zcorn, updatedACTNUM );
    } else {
