...
#ifndef EWOMS_SPARSETABLE_HEADER
#define EWOMS_SPARSETABLE_HEADER
...
namespace Ewoms
{
	...
	/// rows as indicated by the given row sizes.
        ...
        {
#ifndef NDEBUG
            EWOMS_ERROR_IF(row < 0 || row >= size(), "Row index " << row << " is out of range");
#endif
	...
	void setRowStartsFromSizes(IntegerIter rowsize_beg, IntegerIter rowsize_end)
	{
            ...
            if (num_rows < 1) {
                EWOMS_THROW(std::runtime_error, "Must have at least one row. Got " << num_rows << " rows.");
            }
#ifndef NDEBUG
            if (*std::min_element(rowsize_beg, rowsize_end) < 0) {
                EWOMS_THROW(std::runtime_error, "All row sizes must be at least 0.");
            }
#endif
            ...
            if (int(data_.size()) != row_start_.back()) {
                EWOMS_THROW(std::runtime_error, "End of row start indices different from data size.");
            }
...
#endif // EWOMS_SPARSETABLE_HEADER
