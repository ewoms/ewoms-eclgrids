...
struct FaceViaCellHandleWrapper
{
    ...
    size(const T&)
    {
        EWOMS_THROW(std::logic_error, "This should never throw! We only know sizes for cells");
        return 1;
    }
    ...
    gather(B&, const T&)
    {
        EWOMS_THROW(std::logic_error, "This should never throw! We can only gather cell values and indicate that");
    }
    template<class B>
