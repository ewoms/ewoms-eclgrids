...
#ifndef EWOMS_CPGRIDDATA_HEADER
#define EWOMS_CPGRIDDATA_HEADER
...
public:
    enum{
        ...
        /// communicating. 1 is big enough for eWoms as we always use
...
#if HAVE_ECL_INPUT
    /// Read the Eclipse grid format ('grdecl').
    /// \param deck the parsed deck from ewoms-eclio (which is a low-level object)
    /// \param periodic_extension if true, the grid will be (possibly) refined, so that
    ///        intersections/faces along i and j boundaries will match those on the other
    ///        side. That is, i- faces will match i+ faces etc.
    ...
    void processEclipseFormat(const Ewoms::Deck& deck, bool periodic_extension, bool turn_normals = false, bool clip_z = false,
    ...
    void processEclipseFormat(const Ewoms::EclipseGrid* ecl_grid, bool periodic_extension, bool turn_normals = false, bool clip_z = false,
                              const std::vector<double>& poreVolume = std::vector<double>(), const Ewoms::NNC& nncs = Ewoms::NNC());
#endif
...
private:
    ...
    Ewoms::SparseTable<int>             face_to_point_;
                ...
                std::tuple<T,T,T,T,T>& interfaces)
{
    ...
    EWOMS_THROW(std::runtime_error, "Invalid Interface type was used during communication");
}
...
#include <ewoms/eclgrids/cpgrid/indexsets.hh>
