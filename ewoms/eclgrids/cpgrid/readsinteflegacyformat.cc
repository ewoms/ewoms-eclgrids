...
namespace Dune
{
    ...
    void cpgrid::CpGridData::readSintefLegacyFormat(const std::string& grid_prefix)
    {
        ...
        std::string topofilename = grid_prefix + "-topo.dat";
        {
            ...
            if (!file) {
                EWOMS_THROW(std::runtime_error, "Could not open file " << topofilename);
            }
        ...
        std::string geomfilename = grid_prefix + "-geom.dat";
        {
            ...
            if (!file) {
                EWOMS_THROW(std::runtime_error, "Could not open file " << geomfilename);
            }
    ...
    namespace
    {
                      ...
                      std::vector<std::array<int,8> >& c2p)
        {
            ...
            if (topo_header.compare(correct_header)) {
                EWOMS_THROW(std::runtime_error, "Header of topology file does not match what we expect, file possibly in wrong format.\n"
            ...
            for (int i = 0; i < num_cells; ++i) {
                ...
                for (int j = 0; j < numf; ++j) {
                    ...
                    if (face2cell[face][ind] != -1) {
                        EWOMS_THROW(std::runtime_error, "Error in creating faces to cells mapping for face " << face);
                    }
            ...
            // Build cell to point
            EWOMS_MESSAGE("Warning: Not yet making a proper cell to point mapping for Sintef legacy grid.");
            c2p.resize(c2f.size());
        } // void readTopo()
...
//                               << codim0 << ':' << 1 << ":volume";
            ...
            if (geom_header.compare(wanted_header)) {
                EWOMS_THROW(std::runtime_error, "Header of geometry file does not match what we expect, file possibly in wrong format.");
            }
            ...
            if (num_faces != num_faces_again) {
                EWOMS_THROW(std::runtime_error, "Inconsistent number of faces in file.");
            }
            ...
            if (num_cells != num_cells_again) {
                EWOMS_THROW(std::runtime_error, "Inconsistent number of cells in file.");
            }
