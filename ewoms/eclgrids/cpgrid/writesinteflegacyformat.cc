...
namespace Dune
{
    ...
    namespace
    {
        void writeTopo(std::ostream& topo,
                       ...
                       const Ewoms::SparseTable<int>& f2p,
    ...
    void cpgrid::CpGridData::writeSintefLegacyFormat(const std::string& grid_prefix) const
    {
        ...
        std::string mapfilename = grid_prefix + "-map.dat";
        {
            ...
            if (!file) {
                EWOMS_THROW(std::runtime_error, "Could not open file " << mapfilename);
            }
            writeMap(file, *this);
        }
        std::string vtkfilename = grid_prefix + "-volumes.vtk";
        {
            ...
            if (!file) {
                EWOMS_THROW(std::runtime_error, "Could not open file " << vtkfilename);
            }
    ...
    namespace
    {
                       ...
                       const int num_points)
        {
            ...
            for (int face = 0; face < num_faces; ++face) {
                Ewoms::SparseTable<int>::row_type fp = f2p[face];
                int nump = fp.size();
                topo << nump;
