...
template<class T>
struct GetRowType<Ewoms::SparseTable<T> >
{
    typedef typename Ewoms::SparseTable<T>::row_type type;
};
...
struct PointGeometryHandle
{
    ...
    gather(B&, const T&)
    {
        EWOMS_THROW(std::logic_error, "This should never throw!");
    }
    template<class B>
...
struct RowSizeDataHandle
{
    ...
    gather(B&, const T&)
    {
        EWOMS_THROW(std::logic_error, "This should never throw. Only available for other codims.");
    }
    template<class B>
...
{
    SparseTableEntity(const Ewoms::SparseTable<int>& table)
        : table_(table)
    {}
...
private:
    const Ewoms::SparseTable<int>& table_;
};

struct SparseTableDataHandle
{
    using Table = Ewoms::SparseTable<int>;
    using DataType = int;
                                      ...
                                      const std::vector<int>& /* cell_part */)
{
    ...
    AttributeDataHandle<Ewoms::SparseTable<EntityRep<1> > >
        face_handle(ccobj_.rank(), *partition_type_indicator_,
                    face_attributes, static_cast<Ewoms::SparseTable<EntityRep<1> >&>(cell_to_face_),
                    *this);
