...
*/
#ifndef EWOMS_CELLQUADRATURE_HH
    ...
    public:
            ...
            : grid_(grid), cell_(cell), degree_(degree)
        {
            if (grid.dimensions > 3) {
                EWOMS_THROW(std::runtime_error, "CellQuadrature only implemented for up to 3 dimensions.");
            }
            if (degree > 2) {
                EWOMS_THROW(std::runtime_error, "CellQuadrature exact for polynomial degrees > 1 not implemented.");
            }
        }
        ...
        void quadPtCoord(const int index, double* coord) const
        {
            ...
            EWOMS_THROW(std::runtime_error, "Should never reach this point.");
        }
