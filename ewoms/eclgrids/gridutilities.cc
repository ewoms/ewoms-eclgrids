...
namespace Ewoms
{
                               ...
                               SparseTable<int>& nb)
    {
        if (grid.dimensions != 2) {
            EWOMS_THROW(std::logic_error, "Cannot use orderCounterClockwise in " << grid.dimensions << " dimensions.");
        }
        const int num_cells = grid.number_of_cells;
        if (nb.size() != num_cells) {
            EWOMS_THROW(std::logic_error, "Inconsistent arguments for orderCounterClockwise().");
        }
