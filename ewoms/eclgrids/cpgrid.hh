...
#ifndef EWOMS_CPGRID_HEADER
#define EWOMS_CPGRID_HEADER
...
#include "cpgrid/intersection.hh"
#include "cpgrid/entity.hh"
#include "cpgrid/geometry.hh"
#include "cpgrid/iterators.hh"
#include "cpgrid/indexsets.hh"
#include "cpgrid/defaultgeometrypolicy.hh"
#include "common/volumes.hh"
...
namespace Dune
{
    ...
    public:
        ...
        ///
        /// It provides additional methods not in the DUNE interface but needed by eWoms.
            ...
            if (current_view_data_->face_to_cell_[f].size() == 1) {
                if (current_view_data_->uniqueBoundaryIds()) {
                    ...
                    case NNC_FACE:
                        ...
                        EWOMS_THROW(std::logic_error, "NNC face at boundary. This should never happen!");
                    }
                }
            }
            return ret;
        ...
        /// by iterating over Ewoms::UgGridHelpers::cell2Faces(grid).
        ...
        faceTag(const Cell2FacesRowIterator& cell_face) const
        {
            ...
            case NNC_FACE:
                ...
                return -1;
            default:
                EWOMS_THROW(std::logic_error, "Unhandled face tag. This should never happen!");
            }
        }
        ...
        void scatterData(DataHandle& handle) const
        {
            ...
            if(!distributed_data_)
                EWOMS_THROW(std::runtime_error, "Moving Data only allowed with a load balanced grid!");
...
#if HAVE_MPI
            if(!distributed_data_)
                EWOMS_THROW(std::runtime_error, "Moving Data only allowed with a load balance grid!");
        ...
        {
            if (! distributed_data_)
                EWOMS_THROW(std::logic_error, "No distributed view available in grid");
...
#include <ewoms/eclgrids/cpgrid/cartesianindexmapper.hh>
#endif // EWOMS_CPGRID_HEADER
